...
public class ReportCommand extends SubCommand {
    ...
    public void execute(CommandContext<ServerCommandSource> context) {
        ...
        if (message.isBlank()) {
            context.getSource().sendFeedback(() -> Text.of("§b[Tebex] §7A message is required for your report."), false);
        } else {
            context.getSource().sendFeedback(() -> Text.of("§b[Tebex] §7Sending your report to Tebex..."), false);
            platform.sendTriageEvent(new Error("User reported error in-game: " + message));
            context.getSource().sendFeedback(() -> Text.of("§b[Tebex] §7Report sent successfully."), false);
        }
    }
